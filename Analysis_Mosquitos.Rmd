---
title: "Does precipitation and temperature play a role in West Nile Virus Cases in Maricopa County?"
...
ideal temperature for mosquitoes to survive is 50-95°F (10-35°C) and the
*Goldilocks* temperature is 75°F (24°C) [^2]
[^3].
...
```{r, echo=FALSE, dev='png', error=FALSE, warning=FALSE}
...
w_2020<- ggplot(west_nile_2020, aes(x = date1, y = cases)) +
  geom_line(color = "blue") +
  scale_x_date(breaks = scales::pretty_breaks(n=10),date_labels = "%b-%Y") +
  labs(x = "Date", title = "2006-2020 reported cases") + theme_classic()
...
In the relationship between two time series (y,x) the series y 
may be related to past lags of the x-series. 

```{r, echo=TRUE}
# Cross Correlation Functions CCF for unstable
png(filename = "CCF_lag.png")

ccf_values <- ccf(cases_2021$cases, cases_2021$average_prcp)

plot(ccf_values, main = "CCF lag for Cases and Average Precipitation") 
...
m1 <- lm(truncated_cases ~ lag7)

summary(m1)
stargazer(m1, 
          title = "Regression Lag Model Results", 
          type = "text")
